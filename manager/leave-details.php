...
$typeofleave = $_POST['leaveapplied'];
$emp_id = $_POST['emp_id'];
$sql_smt = "SELECT * FROM `tblemployees` WHERE `id` = $emp_id";
...
{  
    if($typeofleave =='Sick'){
        $sl = $result_smt->sl;
        $updateleave = $sl - $leavedays;
        $updatesql="update tblemployees set sl=:sickleave where id=:did";
        $updatequery = $dbh->prepare($updatesql);
        $updatequery->bindParam(':sickleave',$updateleave,PDO::PARAM_STR);
        $updatequery->bindParam(':did',$emp_id,PDO::PARAM_STR);
        $updatequery->execute();
    ...
    if($typeofleave =='Earned'){
        $el = $result_smt->el;
        $updateleave = $el - $leavedays;
        $updatesql="update tblemployees set el=:earnedleave where id=:did";
        $updatequery = $dbh->prepare($updatesql);
        $updatequery->bindParam(':earnedleave',$updateleave,PDO::PARAM_STR);
        $updatequery->bindParam(':did',$emp_id,PDO::PARAM_STR);
        $updatequery->execute();
    ...
    if($typeofleave =='Casual'){
        $cl = $result_smt->cl;
        $updateleave = $cl - $leavedays;
        $updatesql="update tblemployees set cl=:casualleave where id=:did";
        $updatequery = $dbh->prepare($updatesql);
        $updatequery->bindParam(':casualleave',$updateleave,PDO::PARAM_STR);
        $updatequery->bindParam(':did',$emp_id,PDO::PARAM_STR);
        $updatequery->execute();
    }
}
...
{         
  ...
  <tr>
                                            ...
                                            <td><?php $leaveapplied = $result->LeaveType;
                                            echo htmlentities($leaveapplied);
                                            ?></td>
...
<div id="modal1" class="modal modal-fixed-footer" style="height: 60%">
    <div class="modal-content" style="width:90%">
        <h4>Leave take action </h4>
            ...
            <input type="hidden" name="days" value=<?php echo  $daysDifference;?> >
            <input type="hidden" name="leaveapplied" value=<?php echo  $leaveapplied;?> >
